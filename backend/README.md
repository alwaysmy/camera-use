...
`-mcp` 下**相机与旁路都按需启停**，避免常驻占用摄像头：

| | 行为 |
|---|---|
| 进程启动 | 毫秒级（不碰相机） |
| 首次需要画面 | 自动开相机（1~2s） |
| `camera_look` 默认 | 不启动 YOLO，只拍照 |
| `camera_look {detect:true}` / `observe {what:"face/pose/hand"}` | 按需拉起旁路（首次 5~10s 加载模型） |
| 空闲 60s / 120s | 相机自动释放 / 旁路自动关闭 |

HTTP（WebUI）模式保持常开（用户正在看画面，不该关）。
